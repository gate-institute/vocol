...
var endpoint = app.get('fusekiURL') + (app.get('fusekiPortNumber') || 3030) 
...
fs.exists(path, function (exists) {
  if (exists) {
    jsonfile.readFile(path, function (err, obj) {
      ...
      shell.exec('curl -X PUT -H "Authorization: Basic $(echo -n admin:indasp123! | base64)"  -H Content-Type:text/turtle -G ' +endpoint+ '/connectorData  -T  ../serializations/SingleVoc.nt    --data-urlencode graph=default   ', {
        silent: false
      });
      console.log('curl -X PUT -H "Authorization: Basic $(echo -n admin:indasp123! | base64)"  -H Content-Type:text/turtle -G ' + endpoint + '/connectorData  -T  ../serializations/SingleVoc.nt    --data-urlencode graph=default   ')
        shell.exec('pwd');
        shell.cd('../../../.').stdout;
      }
    });
